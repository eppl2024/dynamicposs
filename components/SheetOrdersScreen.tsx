...
interface SheetOrdersScreenProps {
  sheetId: string;
}

export default function SheetOrdersScreen({ sheetId }: SheetOrdersScreenProps) {
  const { user } = useAuth();
  const {
    products,
    productCategories,
  ...
  const currentSheet = googleSheets.find(sheet => sheet.id === sheetId);

  useEffect(() => {
    // Check if this sheet is the active one
    setIsSheetActive(activeSheet?.id === sheetId);
    
    // If this sheet is not active, switch to it
    if (currentSheet && activeSheet?.id !== sheetId) {
      setActiveSheet(sheetId);
    }
  }, [sheetId, activeSheet, currentSheet]);

  useEffect(() => {
    if (isSheetActive && currentSheet?.url && products.length === 0) {
      loadProducts();
    }
  }, [isSheetActive, currentSheet?.url]);
      ...
      <LinearGradient colors={['#e3f2fd', '#fce4ec']} style={styles.container}>
        <SafeAreaView style={styles.safeArea}>
          <View style={styles.errorContainer}>
            <Text style={styles.errorTitle}>Sheet Not Found</Text>
            <Text style={styles.errorText}>
              The requested Google Sheet could not be found.
            </Text>
          </View>
        </SafeAreaView>
      </LinearGradient>
      ...
      <LinearGradient colors={['#e3f2fd', '#fce4ec']} style={styles.container}>
        <SafeAreaView style={styles.safeArea}>
          <View style={styles.loadingContainer}>
            <Text style={styles.loadingText}>Switching to {currentSheet.name}...</Text>
          </View>
        </SafeAreaView>
      </LinearGradient>
    );
    ...
    <LinearGradient colors={['#e3f2fd', '#fce4ec']} style={styles.container}>
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
          <Text style={styles.title}>{currentSheet.name}</Text>
